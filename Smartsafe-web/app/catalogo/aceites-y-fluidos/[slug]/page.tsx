...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    const product = await getProductBySlug('aceites-y-fluidos', slug);
    if (!product) return {};
    return {
...
export default async function AceitesProductPage({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const product = await getProductBySlug('aceites-y-fluidos', slug);
            ...
            <div className="border-b border-white/5 bg-zinc-900/50 backdrop-blur-md sticky top-14 z-40">
                <Container>
                    <div className="flex items-center gap-2 py-4 text-sm text-zinc-400">
                        ...
                        <Link href="/catalogo/aceites-y-fluidos" className="hover:text-white transition-colors">Aceites y Fluidos</Link>
                        <ChevronRight size={14} />
